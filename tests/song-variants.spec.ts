...
test.describe("song variants", () => {
  test("default route plays the configured song and honors query params", async ({ page }) => {
    ...
    expect(src).toContain("ELLIE%20GOULDING%20CUT%203");
  ...
  test("/sesi2 plays the configured song with friend time", async ({ page }) => {
    await openAndAssertAudio(page, "/sesi2", "ELLIE%20GOULDING%20CUT%203");
    await expect(page.getByText("12:30 - 14:00 WIB")).toBeVisible();
  });

  test("/family plays the configured song with family time", async ({ page }) => {
    await openAndAssertAudio(page, "/family", "ELLIE%20GOULDING%20CUT%203");
    await expect(page.getByText("10:30 - 12:00 WIB")).toBeVisible();
  });
});
